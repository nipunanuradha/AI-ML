...
for i = 1:10
    ...
    if i < 10
        file_name = sprintf('U0%d_Acc_TimeD_MDay.mat', i);
    else
        file_name = sprintf('U%d_Acc_TimeD_MDay.mat', i);
    end
    ...
    if isfile(file_name)
        ...
        if isfield(data, 'Acc_TD_Feat_Vec')
            combined_all_data{i} = [data.Acc_TD_Feat_Vec, i * ones(size(data.Acc_TD_Feat_Vec, 1), 1)];
        else
            error('File %s does not contain the variable "Acc_TD_Feat_Vec". Please check the file.', file_name);
        end
    else
